...
public enum JavaFxObservable {
   ...
   */
  public static <T extends Event> Observable<T> eventsOf(final Node node,
   ...
   * @param <T>          the type of the observed value
   * @return an Observable emitting values as the wrapped ObservableValue changes, null will be
   * replaces with nullSentinel
   */
  public static <T> Observable<T> valuesOf(final ObservableValue<T> fxObservable,
      final T nullSentinel) {
   ...
   */
  public static <T> Observable<Optional<T>> nullableValuesOf(
      final ObservableValue<T> fxObservable) {
    return ObservableValueSource.fromNullableObservableValue(fxObservable);
  }

  /**
   * Create an rx Observable from a javafx ObservableValue, and emits changes with old and new value
   * pairs
   *
   * @param fxObservable the observed ObservableValue
   * @param <T>          the type of the observed value
  ...
  /**
   * Create an rx Observable from a javafx ObservableValue, and emits changes with non-null old and
   * new value pairs
   *
   * @param fxObservable the observed ObservableValue
   * @param <T>          the type of the observed value
   ...
   * @return
   */
  public static Observable<javafx.beans.Observable> invalidationsOf(
      final javafx.beans.Observable fxObservable) {
   ...
   * @return An Observable of UI events, appropriately typed
   */
  public static <T extends Event> Observable<T> eventsOf(final Scene scene,
      final EventType<T> eventType) {
   ...
   * @return An Observable of UI events, appropriately typed
   */
  public static <T extends WindowEvent> Observable<T> eventsOf(final Window window,
      final EventType<T> eventType) {
  ...
  /**
   * Creates an observable that emits all updated items from an ObservableList. If you declare an
   * ObservableList that listens to one or more properties of each element, you can emit the changed
   * items every time these properties are modified
   *
  ...
  /**
   * Emits distinctly  added and removed items from an ObservableList. If dupe items with identical
   * hashcode/equals evaluations are added to an ObservableList, only the first one will fire an
   * ADDED item. When the last dupe is removed, only then will it fire a REMOVED item.
   *
   * @param source
   * @return An Observable emitting changed items with an ADDED, REMOVED, or UPDATED flag
   */
  ...
  /**
   * Emits distinctly added and removed T items from an ObservableList based on a mapping to an R
   * value. If dupe mapped R items with identical hashcode/equals evaluations are added to an
   * ObservableList, only the first one will fire an ADDED T item. When the last R dupe is removed,
   * only then will it fire a REMOVED T item.
   *
   * @param source
   * @return An Observable emitting changed items with an ADDED, REMOVED, or UPDATED flag
   */
  public static <T, R> Observable<ListChange<T>> distinctChangesOf(final ObservableList<T> source,
      Function<T, R> mapper) {
    return ObservableListSource.fromObservableListDistinctChanges(source, mapper);
  }

  /**
   * Emits distinctly added and removed mappings to each R item from an ObservableList. If dupe
   * mapped R items with identical hashcode/equals evaluations are added to an ObservableList, only
   * the first one will fire an ADDED R item. When the last dupe is removed, only then will it fire
   * a REMOVED R item.
   *
   * @param source
   * @return An Observable emitting changed mapped items with an ADDED, REMOVED, or UPDATED flag
   */
  public static <T, R> Observable<ListChange<R>> distinctMappingsOf(final ObservableList<T> source,
      Function<T, R> mapper) {
   ...
   */
  public static <K, T> Observable<ObservableMap<K, T>> emitOnChanged(
      final ObservableMap<K, T> source) {
  ...
  /**
   * Returns an Observable that emits a 0L  and ever increasing numbers after each duration of time
   * thereafter
   */
  ...
  /**
   * Returns a Maybe that emits the T response  of a Dialog. If no response was given then the Maybe
   * will be empty.
   */
